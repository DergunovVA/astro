...
class TestNewFeatures:
    ...
    def test_natal_koch_houses(self):
        ...
        result = self.run_command("natal", "1985-01-15", "14:30", "Moscow", "--house-system", "Koch")
        ...
        result_placidus = self.run_command("natal", "1985-01-15", "14:30", "Moscow", "--house-system", "Placidus")
        cusps_koch = [float(f["value"]) for f in house_cusps]
        cusps_placidus = [float(f["value"]) for f in result_placidus["facts"] if f["type"] == "house_cusp"]
        # At least some should be different (they're different systems)
        assert len([i for i, (k, p) in enumerate(zip(cusps_koch, cusps_placidus)) if abs(k - p) > 0.1]) > 0

    def test_natal_whole_sign_houses(self):
        """Test natal with Whole Sign house system."""
        result = self.run_command("natal", "1985-01-15", "14:30", "Moscow", "--house-system", "Whole Sign")
    ...
    def test_synastry_command(self):
        ...
        result = self.run_command(
            "synastry",
            "1990-05-15", "14:30", "Moscow",
            "1992-03-20", "10:15", "London"
        )
    ...
    def test_natal_different_house_systems_produce_different_results(self):
        ...
        placidus = self.run_command("natal", "1985-01-15", "14:30", "Moscow", "--house-system", "Placidus")
        koch = self.run_command("natal", "1985-01-15", "14:30", "Moscow", "--house-system", "Koch")
        ...
        differences = sum(1 for p, k in zip(placidus_cusps, koch_cusps) if abs(p - k) > 0.1)
