...
class RedTeamTester:
    ...
    def print_summary(self):
        ...
        print(
            f"Passed: {self.passed} ({100 * self.passed // self.total if self.total else 0}%)"
        )
...
for date, time, place in test_cases:
    ...
    try:
        ...
        print_result(
            "Empty input rejected", False, f"Accepted: {repr((date, time, place))}"
        )
        tester.log_issue(
            "HIGH",
            "Input Validation",
        ...
        print_result("Empty input rejected", True, "ValueError raised")
        tester.passed += 1
    except Exception as e:
        print_result("Empty input rejected", False, f"Unexpected: {type(e).__name__}")
        tester.log_issue(
            "CRITICAL",
            "Error Handling",
...
for date in invalid_dates:
    ...
    try:
        ...
        print_result("Invalid date rejected", False, f"Accepted: {date}")
        tester.log_issue("HIGH", "Input Validation", f"Invalid date accepted: {date}")
    except ValueError:
        print_result("Invalid date rejected", True)
        tester.passed += 1
    except Exception as e:
        print_result("Invalid date rejected", False, f"{type(e).__name__}")
        tester.log_issue(
            "CRITICAL", "Error Handling", f"Crash on invalid date {date}: {e}"
        )
...
for time in invalid_times:
    ...
    try:
        ...
        print_result("Invalid time rejected", False, f"Accepted: {time}")
        tester.log_issue("HIGH", "Input Validation", f"Invalid time accepted: {time}")
    except ValueError:
        print_result("Invalid time rejected", True)
        tester.passed += 1
    except Exception as e:
        print_result("Invalid time rejected", False, f"{type(e).__name__}")
...
try:
    ...
    if coords1 == coords2:
        print_result("Cache consistency", True, "Same coords returned")
        tester.passed += 1
    else:
        print_result("Cache consistency", False, f"{coords1} != {coords2}")
        tester.log_issue(
            "HIGH",
            "Data Integrity",
    ...
    print_result("Cache consistency", False, str(e))
...
try:
    ...
    print_result("Cache reset works", True, "No exception")
    tester.passed += 1
except Exception as e:
    print_result("Cache reset works", False, str(e))
...
try:
    ...
    if len(set(moscow_coords)) == 1:
        print_result("Concurrent access (simulated)", True)
        tester.passed += 1
    else:
        print_result("Concurrent access (simulated)", False, "Inconsistent results")
        tester.log_issue(
            "MEDIUM", "Concurrency", "Inconsistent coordinates in concurrent simulation"
        )
except Exception as e:
    print_result("Concurrent access (simulated)", False, str(e))
...
try:
    ...
    if abs(diff - expected_diff) < 1e-8:
        print_result("JD precision (1 second)", True, f"Diff: {diff:.10f}")
        tester.passed += 1
    else:
        print_result(
            "JD precision (1 second)", False, f"Expected ~{expected_diff}, got {diff}"
        )
        tester.log_issue(
            "MEDIUM",
            "Precision",
    ...
    print_result("JD precision", False, str(e))
...
try:
    ...
    if os.path.exists(cache_file):
        ...
        try:
            ...
            print_result("Corrupted cache recovery", True, "Recovered gracefully")
            tester.passed += 1
        except Exception as e:
            print_result("Corrupted cache recovery", False, f"Failed: {e}")
            tester.log_issue(
                "CRITICAL", "Resilience", f"No recovery from corrupted cache: {e}"
            )
        ...
        print_result("Corrupted cache recovery", False, "No cache file found")
except Exception as e:
    print_result("Corrupted cache recovery", False, str(e))
...
print_result("Missing dependency handling", False, "Not tested (requires mocking)")
...
try:
    ...
    if per_cmd < 100:  # Should be < 100ms per command with cache
        print_result("Batch processing (100 cmds)", True, f"{per_cmd:.1f}ms/cmd")
        tester.passed += 1
    else:
        print_result(
            "Batch processing (100 cmds)", False, f"{per_cmd:.1f}ms/cmd (expected <100)"
        )
        tester.log_issue(
            "MEDIUM", "Performance", f"Slow command processing: {per_cmd:.1f}ms/cmd"
        )
except Exception as e:
...
try:
    ...
    print_result("Memory pressure (10 cities)", True, f"Cached {added} unique entries")
    tester.passed += 1
except Exception as e:
    print_result("Memory pressure", False, str(e))
...
try:
    ...
    ni_msk = normalize_input(
        "1990-01-01", "12:00", "Moscow", tz_override="Europe/Moscow"
    )  # UTC+3

    # Should represent same moment
    if ni_utc.utc_dt == ni_msk.utc_dt:
        print_result("TZ offset consistency", True, "Same UTC time detected")
        tester.passed += 1
    else:
        print_result(
            "TZ offset consistency",
            False,
    ...
    print_result("TZ offset consistency", False, str(e))
...
try:
    ...
    print_result("DST handling", False, "Not fully tested (requires mocking)")
except Exception as e:
    print_result("DST handling", False, str(e))
...
for date in boundary_dates:
    try:
        ...
        print_result(f"Boundary: {date}", True)
        tester.passed += 1
        tester.total -= 1  # Adjust count
    except Exception as e:
        print_result(f"Boundary: {date}", False, str(e)[:30])
        boundary_ok = False
