    ...
    if strong_ids or weak_ids:
        ...
        signals.append(
            Signal(
                ...
                weight=round(
                    len(strong_ids) / max(len(strong_ids) + len(weak_ids), 1), 2
                ),
            )
        )
    ...
    if angular_ids:
        ...
        signals.append(
            Signal(
                ...
                weight=round(
                    len(angular_ids) / max(len(angular_ids) + len(cadent_ids), 1), 2
                ),
            )
        )
    ...
    if element_counts[dominant_elem] >= 3:
        signals.append(
            Signal(
                ...
                weight=round(
                    element_counts[dominant_elem]
                    / max(sum(element_counts.values()), 1),
                    2,
                ),
            )
        )
...
def decisions_from_signals(signals: List[Signal]) -> List[Decision]:
    ...
    if intensity_sig:
        if intensity_sig.intensity == "high":
            ...
            recommendation = (
                "Дополнительный вызов и дисциплина помогут реализовать потенциал."
            )
        decisions.append(
            Decision(
        ...
        if elem_sig:
            ...
            decisions.append(
                Decision(
                    ...
                    recommendation="Уделите внимание развитию недостающих стихий для баланса.",
                    fatal=False,
                )
            )
